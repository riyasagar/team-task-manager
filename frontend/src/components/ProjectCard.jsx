...
export default function ProjectCard({ project }) {
  ...
  const getMemberRole = (memberId) => {
    ...
    const role =
      project.roles[memberId] || project.roles[String(memberId)];
    return role || "Member";
  };
    ...
    <Link to={`/projects/${project?._id}`} className="block group">
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-xl hover:border-indigo-300 hover:-translate-y-1">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          >
            <div className="flex items-center gap-1.5 rounded-full bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 px-3 py-1.5 cursor-pointer">
              <Users className="h-3.5 w-3.5 text-emerald-600" />
              <span className="text-xs font-semibold text-emerald-700">
                {toArray(project?.members).length}
              </span>
            </div>

            {/* Tooltip */}
            {showMembersTooltip &&
              toArray(project?.members).length > 0 && (
                <div className="absolute right-0 top-full mt-2 z-50 w-56 rounded-xl border border-slate-200 bg-white shadow-lg p-3">
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    ...
                    {/* ✅ FIXED MAP */}
                    {toArray(project?.members).map((member) => {
                      ...
                      return (
                        <div key={memberId} className="flex items-center gap-2">
                          ...
                          {isAdmin ? (
                            ...
                            <span className="flex items-center gap-1 rounded bg-slate-100 px-1.5 py-0.5 text-xs font-medium text-slate-600">
        ...
        <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
          ...
          <span className="flex items-center gap-1 rounded-lg bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700">
            View
